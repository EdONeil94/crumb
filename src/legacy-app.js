...
const DEFAULT_DIM_5TH = { key: 'dim_sweetness', label: 'Sweetness', tip: 'The balance of sugar' };

function getTastingDims(category) {
...
// Legacy flat list for any code that still references TASTING_DIMS
const TASTING_DIMS = ['Appearance', 'Texture', 'Flavour', 'Value', 'dim5'];
...
async function refreshReactionBar(itemId) {
  const { db, collection, query, where, getDocs } = fb;
...
function buildReactionBarInner(itemId, reactions) {
  // Group by emoji
  const counts = {};
  const userReacted = {};
  reactions.forEach(r => {
...
// ─── QR CODE ──────────────────────────────────────────────────────────────────
function generateOrderQRCodes(reservations) {
  reservations.forEach(r => {
...
async function openQRScanner(bakeryName) {
  // Build overlay
  const overlay = document.createElement('div');
...
async function processScannedReservation(reservationId, bakeryName) {
  if (!fb) return;
  const { db, doc, getDoc, updateDoc } = fb;
