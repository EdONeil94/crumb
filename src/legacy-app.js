...
function computeCountryRank(uid, country) {
...
function populateRankingLocationFilter() {
...
// ── Baker: manage offerings ────────────────────────────────────────────────────
async function openManagePreordersModal(bakeryName) {
  if (!ownsBakery(bakeryName)) return;
...
function switchMpTab(tab, bakeryName) {
...
async function renderMpTab(tab, bakeryName) {
  const content = document.getElementById('mpTabContent');
...
function closeManagePreordersModal() {
  document.getElementById('managePreordersModal').classList.remove('open');
  unlockScroll();
}

async function renderMpUpcoming(panel, bakeryName) {
...
async function renderMpHistoric(panel, bakeryName, days) {
  if (!panel) panel = document.getElementById('mpTabContent');
  const { db, collection, query, where, getDocs } = fb;
  const now = new Date();
...
async function renderMpMonth(panel, bakeryName) {
  if (!panel) panel = document.getElementById('mpTabContent');
  const { db, collection, query, where, getDocs } = fb;
  const now = new Date();
...
async function renderMpForecast(panel, bakeryName) {
  if (!panel) panel = document.getElementById('mpTabContent');
  const { db, collection, query, where, getDocs } = fb;
  const now = new Date();
  const todayStr = now.toISOString().split('T')[0];
...
function mpItemBreakdownHTML(reservations, totalRevenue) {
...
function showMpDayDetail(dateStr, bakeryName) {
...
async function openEditOffering(offeringId, bakeryName) {
  if (!fb) return;
  const { db, doc, getDoc } = fb;
...
async function saveEditOffering(offeringId, bakeryName) {
  if (!fb) return;
  const { db, doc, updateDoc } = fb;
...
function showAddOfferingForm(bakeryName) {
...
async function loadBakeryCatalogue(bakeryName) {
  if (!fb) return;
...
async function saveToCatalogue(bakeryName, name, description, price, photoURL) {
...
async function saveOffering(bakeryName) {
  if (!fb || !currentUser) return;
...
async function deleteOffering(offeringId, bakeryName) {
...
async function markCollected(reservationId, bakeryName) {
  const { db, doc, updateDoc } = fb;
  try {
...
async function openCatalogueManager(bakeryName) {
  const overlay = document.createElement('div');
...
// closeQRScanner/processScannedReservation moved to
// src/components/qrCode.js (2026-08-24, Phase 2 step 10) — imported above.
// confirmCollected/closeQrConfirmOverlay stay here, deferred — see
// CLAUDE.md's own callout for why and when to revisit.
...
// confirmCollected had no call sites outside this cluster, so it comes out
// of WINDOW EXPORTS entirely. closeQrConfirmOverlay is new. closeQRScanner
// registers from src/components/qrCode.js now (Phase 2 step 10).
registerActions({ closeQrConfirmOverlay, confirmCollected });
...
// openEditOffering/deleteOffering are the baker-side Manage Pre-orders
// modal — converted separately below. expandQR/closeExpandedQR (QR-code
// tap-to-enlarge and its close button) register from
// src/components/qrCode.js now (Phase 2 step 10).
...
// `this.value`, since it had exactly one call site. All 15 functions below
...
// EXPORTS below since that was its only reason for being there. openQRScanner
// (the "📷 Scan QR" button) registers from src/components/qrCode.js now
// (Phase 2 step 10).
registerActions({
